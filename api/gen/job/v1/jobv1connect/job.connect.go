...
import (
	...
	context "context"
	errors "errors"
	v1 "github.com/ei-sugimoto/yamicheck/api/gen/job/v1"
	http "net/http"
	strings "strings"
)
...
// period.
const (
	// JobServiceCheckProcedure is the fully-qualified name of the JobService's Check RPC.
	JobServiceCheckProcedure = "/job.v1.JobService/Check"
)

// These variables are the protoreflect.Descriptor objects for the RPCs defined in this package.
var (
	jobServiceServiceDescriptor     = v1.File_job_v1_job_proto.Services().ByName("JobService")
	jobServiceCheckMethodDescriptor = jobServiceServiceDescriptor.Methods().ByName("Check")
)
...
func NewJobServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) JobServiceClient {
	...
	return &jobServiceClient{
		check: connect.NewClient[v1.CheckRequest, v1.CheckResponse](
			httpClient,
			baseURL+JobServiceCheckProcedure,
			connect.WithSchema(jobServiceCheckMethodDescriptor),
...
// Check calls job.v1.JobService.Check.
func (c *jobServiceClient) Check(ctx context.Context, req *connect.Request[v1.CheckRequest]) (*connect.Response[v1.CheckResponse], error) {
...
func NewJobServiceHandler(svc JobServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	jobServiceCheckHandler := connect.NewUnaryHandler(
		JobServiceCheckProcedure,
		svc.Check,
		connect.WithSchema(jobServiceCheckMethodDescriptor),
	...
	return "/job.v1.JobService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case JobServiceCheckProcedure:
			jobServiceCheckHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
...
func (UnimplementedJobServiceHandler) Check(context.Context, *connect.Request[v1.CheckRequest]) (*connect.Response[v1.CheckResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("job.v1.JobService.Check is not implemented"))
}
